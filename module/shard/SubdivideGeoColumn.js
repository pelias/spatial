...
class SubdivideGeoColumn extends SqliteIndex {
  create (db, config) {
    let introspect = new SqliteIntrospect(db)
    let hasGeoColumn = introspect.columns('shard_subdivide').some(c => c.name === 'geom')
    ...
    if (!hasGeoColumn) {
      ...
      try {
        let res = db.prepare(`
          SELECT AddGeometryColumn('shard_subdivide', 'geom', 4326, 'GEOMETRY', 'XY', 1)
        `).get()
      ...
      try {
        db.prepare(`
          SELECT CreateSpatialIndex('shard_subdivide', 'geom')
      `).get()
      } catch (e) {
  ...
  drop (db, config) {
    ...
    if (hasGeoColumn) {
      ...
      try {
        db.prepare(`
          SELECT DisableSpatialIndex('shard_subdivide', 'geom')
      `).get()
      } catch (e) {
      ...
      try {
        let res = db.prepare(`
          SELECT DiscardGeometryColumn('shard_subdivide', 'geom')
        `).get()
        if (_.get(config, 'verbose', false) === true) { console.error(res) }
      } catch (e) {
...
module.exports = SubdivideGeoColumn
